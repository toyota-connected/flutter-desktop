...
const char* GetLocaleStringFromEnvironment() {
  const char* retval = getenv("LANGUAGE");
...
void AppendLocaleVariants(std::vector<LanguageInfo>& languages,
  ...
  const int mask = ComputeVariantMask(info);
...
std::vector<LanguageInfo> GetPreferredLanguageInfo() {
  const char* locale_string = GetLocaleStringFromEnvironment();
