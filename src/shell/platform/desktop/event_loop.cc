...
EventLoop::TaskTimePoint EventLoop::TimePointFromFlutterTime(
    const uint64_t flutter_target_time_nanos) {
  const auto now = TaskTimePoint::clock::now();
  const int64_t flutter_duration =
      flutter_target_time_nanos - LibFlutterEngine->GetCurrentTime();
  return now + std::chrono::nanoseconds(flutter_duration);
}

void EventLoop::PostTask(const FlutterTask flutter_task,
                         const uint64_t flutter_target_time_nanos) {
