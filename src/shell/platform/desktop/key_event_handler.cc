...
UTF8CodePointInfo GetUTF8CodePointInfo(const int first_byte) {
...
bool GetUTF32CodePointFromGLFWKey(const int key,
                                  const int scan_code,
                                  uint32_t* code_point) {
  ...
  int shift = static_cast<int>(byte_info.length) - 1;
...
void KeyEventHandler::CharHook(GLFWwindow* /* window */,
