...
#include "shell/platform/desktop/public/flutter_desktop.h"
...
#include "shell/platform/desktop/text_input_plugin.h"
...
static void SendWindowMetrics(
    FlutterDesktopWindowControllerState const* controller,
    const int width,
    const int height) {
  const double dpi = controller->window_wrapper->pixels_per_screen_coordinate *
  ...
  LibFlutterEngine->SendWindowMetricsEvent(controller->engine->flutter_engine,
                                           &event);
}
...
static void SendPointerEventWithData(GLFWwindow* window,
  ...
  const double pixels_per_coordinate =
      controller->window_wrapper->pixels_per_screen_coordinate;
  ...
  LibFlutterEngine->SendPointerEvent(controller->engine->flutter_engine, &event,
                                     1);
...
static bool EngineMakeContextCurrent(void* user_data) {
  FlutterDesktopEngineState const* engine_state =
      static_cast<FlutterDesktopEngineState*>(user_data);
  FlutterDesktopWindowControllerState const* window_controller =
...
UniqueAotDataPtr LoadAotData(const std::filesystem::path& aot_data_path) {
  ...
  const auto result = LibFlutterEngine->CreateAOTData(&source, &data);
  if (result != kSuccess) {
...
static bool RunFlutterEngine(
  ...
  auto result = LibFlutterEngine->Run(FLUTTER_ENGINE_VERSION, &config, &args,
                                      engine_state, &engine);
...
static void SetUpLocales(FlutterDesktopEngineState const* state) {
  const std::vector<flutter::LanguageInfo> languages =
  ...
  const FlutterEngineResult result = LibFlutterEngine->UpdateLocales(
...
FlutterDesktopWindowControllerRef FlutterDesktopCreateWindow(
  ...
  if (!LibFlutterEngine::IsPresent(engine_properties.engine_path)) {
    std::cerr << "libflutter_engine.so not found." << std::endl;
    return nullptr;
  }
  ...
  state->window = UniqueGLFWwindowPtr(
      glfwCreateWindow(window_properties.width, window_properties.height,
                       window_properties.title, nullptr, nullptr),
      glfwDestroyWindow);
  glfwDefaultWindowHints();
...
void FlutterDesktopWindowSetPixelRatioOverride(
  ...
  if (width_px > 0 && height_px > 0) {
    auto const* controller = GetWindowController(flutter_window->window);
    SendWindowMetrics(controller, width_px, height_px);
  }
}

void FlutterDesktopWindowSetSizeLimits(FlutterDesktopWindowRef flutter_window,
                                       const FlutterDesktopSize minimum_size,
                                       const FlutterDesktopSize maximum_size) {
...
FlutterDesktopEngineRef FlutterDesktopRunEngine(
  ...
  auto event_loop = std::make_unique<flutter::HeadlessEventLoop>(
      ...
      [state = engine_state.get()](const auto* task) {
        if (LibFlutterEngine->RunTask(state->flutter_engine, task) !=
            kSuccess) {
          std::cerr << "Could not post an engine task." << std::endl;
        }
      });
...
bool FlutterDesktopShutDownEngine(FlutterDesktopEngineRef engine) {
  const auto result = LibFlutterEngine->Shutdown(engine->flutter_engine);
  delete engine;
